...
public interface ApplicationEntityMapper extends BaseMapper<ApplicationEntity, Integer> {
	
	@Cache(uniqueIndex = true)
	@Select("SELECT * FROM apps  where app_key=#{appKey} limit 1")
	@ResultMap("BaseResultMap")
	ApplicationEntity findByAppKey(String appKey);
	
	@Select("SELECT * FROM apps  where service_id=#{serviceId} limit 1")
	@ResultMap("BaseResultMap")
	ApplicationEntity findByServiceId(String serviceId);
	
	@Select("SELECT * FROM apps  where master_uid=#{masterUid} AND enabled = 1")
	@ResultMap("BaseResultMap")
	List<ApplicationEntity> findByMaster(Integer masterUid);
	...
	List<ApplicationEntity> findByQueryParams(Map<String, Object> param);

	@Select("SELECT * FROM apps  where group_id=#{groupId} AND enabled = 1")
	@ResultMap("BaseResultMap")
	List<ApplicationEntity> findByGroupId(Integer groupId);
}