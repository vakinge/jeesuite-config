			...
			dataRender: function($container,data){
				...
				$('[bindAttr]',$container).each(function(){
					...
					if(bindAttr.indexOf('{')==0){
						...
						try{value = eval('data.'+bindAttr);value = value.toString();}catch(e){}
						if(!value)return;
						if(self.is('input')){
...
//				}
		    ...
		    blockUtil: function(millisecond,func){
	  		...
	  		/*勿删 在openAPI授权列表的编辑功能里用到，后续优化*/
	  		cascadeRendSelect: function (form,$this,url) {
...
exports('jeesuitelayui', jeesuitelayui);
	 ...
	 $('*[dataLoad]').each(function(){
			...
			function renderPage(params){
				...
				$.ajax({
					...
					error: function(xhr, type){
						if(xhr.status == 401 || xhr.status == 403){top.location.href = "/login.html";return;}
						$this.html('系统错误');
					}
				});
			}
  ...
  //submit
	$('body').on('click','.J_ajaxSubmit',function(){
		...
		for(var name in dataParams){
			var replace = '{' + name + '}';
			requestURI = requestURI.replace(replace,dataParams[name]);
		}
	...
	$('body').on('click','.J_form_dialog', function(){
		var self = $(this),
		     ...
		     dataRender = self.attr('data-render'),
		...
		$.get(templdateurl, null, function(f) {
			addBoxIndex = layer.open({
				...
				success: function(layero, index) {
					...
					//form.render();
					if(onLoadFinished){
						eval(onLoadFinished+"(form,templdateurl)");
					}
					...
					form.on('submit(submitbtn)', function(data) {
						var loading = layer.load();
						if(onSubmit){
							eval(onSubmit+"(data.field)");
							return false;
						}
						var url = $form.attr('action');
						jeesuitelayui.post(url,data.field,function(){
							jeesuitelayui.success(data.msg || "操作成功");
		...
		form.on('select('+filterName+')',function(data) {
			...
			$.getJSON(url,function(result){
				...
				if(!data || data.length == 0){
					...
					jeesuitelayui.error(eventOpts.noneMessage || '无关联数据');
					form.render('select');
					return;}
				//
